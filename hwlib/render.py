...
def show(parts: Part | dict[str, Part]) -> None:
    ...
    重要: 必ず「ブラウザを接続してから送信」の順で行う。
    スタンドアロンサーバはモデルを保存も再送もしない。送信された瞬間に
    登録済みのブラウザへ転送するだけで、登録が無ければモデルは捨てられる。
    したがって次の場合はロゴ（デフォルト表示）のままになる。
      - ブラウザを開く／更新するのが show() の後だったとき
      - サーバを再起動して、開いていたタブの登録が切れたとき
    映らないときは、ブラウザのタブを F5 で再読み込み（＝再登録）してから
    もう一度 show() を実行する。
    """
