...
# 実測で確定（2026-08-09、ユーザーがノギスで実測）[mm]
# 変換ハブは、このホーン円板の外周に被せるスカートで芯出しする。嵌合の要。
HORN_BOSS_DIA = 16.0   # ホーン円板の外径（図面 IDLER 側 φ16 と一致）
HORN_BOSS_H = 3.0      # ホーン円板がケース前面から出る高さ
