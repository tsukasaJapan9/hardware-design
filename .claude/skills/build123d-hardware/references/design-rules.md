...
| 欲しいもの | 書き方 | 既定値での結果 |
|---|---|---|
| M3 ネジのバカ穴（clearance hole） | `P.hole(3.0, "free")` | 3.60mm |
...
| 規則 | 値 | 根拠 |
|---|---|---|
| 最小肉厚 | **1.2mm**（外周3本） | 2本ではあいだにインフィル（infill）が入らず割れる |
...
（blind hole）の天井など — はブリッジではなくただの天井。サポートか設計変更が必要。

**サポートは付けるものではなく、設計で消すもの。** 有効な順に。

- **オーバーハングの下に面取り。** ボスやリップの下の 45° 面取りは、追加コストなしで済む
- **横向きの丸穴はティアドロップ（teardrop）に。** 垂直壁の丸穴は頂点が 90° になる。
  φ5 以下はブリッジで渡るので形を崩す価値なし。大径はティアドロップ
- **2分割してネジ留め。** それぞれ平置き・サポートなし・良い層方向で刷れる2部品は、
  無理をした1部品に常に勝る
- **姿勢の変更。** 追加コストなし。たいてい正解だが、層方向への影響を先に確認
...
ネジ頭は `CounterBore`（座ぐり）か `CounterSink`（皿もみ）で沈め、面一（flush）にする。
皿もみは上向きに開く限り自己支持。
...
空洞とぴったり同寸のフタは重なりゼロ。干渉チェックは通る。そして造形板からは 65.15mm で
出てきて、入らない。
...
背後にある経験則: **何かに入るものには、意図して選んだすきまが必要。** フタやカバーは
...
- ポケットが受けるのは**外輪**。`BearingPocket` は座面の穴で**内輪を逃がす**ので、座面が
  回転側に触れることはない
- 圧入時に押すのも外輪だけ。ソケットか万力の平面で。内輪を押すと玉に圧痕が付き、その
  ベアリングは使えなくなる。回転が引っかかるようになり、元には戻らない
